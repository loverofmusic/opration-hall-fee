...
const Cfg = {
  ...
  colorData: {
    //配色方案，部分色彩参考 http://rmco.jp/coloringroom/haisyoku_rei/haisyoku_haru.html
    default: [
      "lightskyblue",
      "orange",
      "greenyellow",
    ...
    spring: ["#BEDC6E", "#FA8C8C", "#FAAAC8", "#FAC8C8", "#FFFFE6", "#6E6464"],
    summer: ["#FFAE00", "#FF5200", "#007AFF", "#00BF05", "#DCFFFF", "#505064"],
    autumn: ["#c1ad2f", /*'#A5912D',*/ "#782323", "#783723", "#A05027", "#FAE6DC", "#283C14"],
    winter: ["#F5F5FA", "#96822D", "#6E5A19", "#BECDEB", "#E1E1F0", "#281E1E"]
  }
};
...
const Public = {
  ...
  getWeather(currTime) {
    ...
    $.get({
      ...
      success: function(data) {
        ...
        $("#weatherIcon").css(
          "background-image",
          `url("https://cdn.heweather.com/cond_icon/${weather2Code[txt]}.png")`
        );
      }
    });
  },
  ...
  //     if (noRefresh && noRefresh.includes(item) && !(someRefresh && someRefresh.includes(item)))
  //       return;
...
$(function() {
    ...
    Object.keys(Cfg.colorData).forEach(item => {
      $colors.append(`
            <label for="colors_${item}">
                <input type="radio" id="colors_${item}" name="colors" ${
        Cfg.colors === item ? "checked" : ""
      }><span>${item}</span>
            </label>
        `);
    });
  ...
  $("body ").on("click", "#saveSetting", function() {
    let settings = {
      ...
      designW: $("#designW").val(),
      designH: $("#designH").val(),
      colors: $("body>aside input[type=radio][name=colors]:checked")
        .next()
        .text(),
      zoomMode: $("body>aside input[type=radio][name=zoomMode]:checked")
        .next()
        .text()
    };
  ...
  // if (
  //   !(sessionData && sessionData.settingTip) &&
  //   !$("body")
  //     .attr("id")
  //     .startsWith("pb_")
  // ) {
